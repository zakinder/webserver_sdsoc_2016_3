proc reinitialise {} {
  ...
  set projectname "NA"
  set BOARD ""
  if {[file exists ${ZQ::BASEFOLDER}/design_basic_settings.cmd]} {
    set fp [open "${ZQ::BASEFOLDER}/design_basic_settings.cmd" r]
    ...
    foreach t $tmp {
      ...
      if {[string match "@set ZIP_PATH=*" $t] } {
        ...
        puts "Info:(ZQ) Restore ZIP path from design_basic_settings.cmd."
      ...
      if {[string match "@set XILINXGIT_DEVICETREE=*" $t] } {
        ...
        puts "Info:(ZQ) Restore XILINXGIT_DEVICETREE path from design_basic_settings.cmd."
      ...
      if {[string match "@set XILINXGIT_UBOOT=*" $t] } {
        ...
        puts "Info:(ZQ) Restore XILINXGIT_UBOOT path from design_basic_settings.cmd."
      ...
      if {[string match "@set XILINXGIT_LINUX=*" $t] } {
        ...
        puts "Info:(ZQ) Restore XILINXGIT_LINUX path from design_basic_settings.cmd."
  ...
  if {[catch {set projectname [get_projects]} result]} {
    ...
    puts "Info:(ZQ) Restore Board variable from design_basic_settings.cmd."
