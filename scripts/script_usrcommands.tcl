namespace eval ZQ {
  ...
  proc help {{args ""}} {
    ...
    for {set option 0} {$option < $args_cnt} {incr option} {
        ...
        "-help"		    { set run_help true; incr option }
          default    { puts "Warning:(ZQ) unrecognised option: [lindex $args $option]"; set run_help true;break }
        }
    }
    if {$run_help} {
      puts "Description:\n\
      ...
      ZQ::INIT::print_version
      puts "------------------------------------------"
      puts "Info:(ZQ) Available ZQ-Functions:"
      if {![catch {set projectname [get_projects]} result]} {
        puts "  ---------------------------------"
        puts "  Beta Test (advanced usage only!):"
        puts "    ZQ::ADV::beta_util_sdsoc_project \[-check_only\] \[-start_sdsoc\] \[-help\]"
        puts "    ZQ::ADV::beta_hw_remove_board_part \[-permanent\] \[-help\]"
        puts "    ZQ::ADV::beta_hw_export_rtl_ip \[-help\]"
      }
      puts "  ----------"
      puts "  Utilities:"
      puts "    ZQ::util_zip_project \[-save_all\] \[-remove_prebuilt\] \[-manual_filename <arg>\] \[-help\]"
      puts "  ------------"
      puts "  Programming:"
      puts "    ZQ::pr_init_hardware_manager \[-help\]"
      puts "    ZQ::pr_program_jtag_bitfile \[-used_board <arg>\] \[-swapp <arg>\] \[-available_apps\] \[-used_basefolder_bitfile\] \[-help\]"
      puts "    ZQ::pr_program_flash_binfile \[-no_reboot\] \[-used_board <arg>\] \[-swapp <arg>\] \[-available_apps\] \[-force_hw_manager\] \[-used_basefolder_binfile\] \[-help\]"
      puts "    ZQ::pr_program_flash_mcsfile \[-no_reboot\] \[-used_board <arg>\] \[-swapp <arg>\] \[-available_apps\] \[-used_basefolder_mcsfile\] \[-help\]"
      if {![catch {set projectname [get_projects]} result]} {
        # on vivado project
        puts "  ----------------"
        puts "  Software Design:"
        puts "    ZQ::sw_run_hsi \[-run_only\] \[-prebuilt_hdf <arg>\] \[-no_hsi\] \[-no_bif\] \[-no_bin\] \[-no_bitmcs\] \[-force_bin\]  \[-clear\] \[-help\]"
        puts "    ZQ::sw_run_sdk \[-open_only\] \[-update_hdf_only\] \[-prebuilt_hdf <arg>\] \[-clear\] \[-help\]"
        puts "  ----------------"
        puts "  Hardware Design:"
        puts "    ZQ::hw_blockdesign_create_bd \[-bd_name\] \[-msys_local_mem\] \[-msys_ecc\] \[-msys_cache\] \[-msys_debug_module\] \[-msys_axi_periph\] \[-msys_axi_intc\] \[-msys_clk\] \[-help\]"
        puts "    ZQ::hw_blockdesign_export_tcl \[-no_mig_contents\] \[-no_validate\] \[-mod_tcl\] \[-svntxt <arg>\]  \[-board_part_only\] \[-help\]"
        puts "    ZQ::hw_build_design \[-disable_bitgen\] \[-disable_hdf\] \[-disable_mcsgen\] \[-disable_reports\] \[-export_prebuilt\] \[-export_prebuilt_only\] \[-help\]"
      }
      puts "------------------------------------------"
      puts "Note:(ZQ) Run only predefined ZQ-functions from this list. Run other ZQ-functions directly may cause errors."
      puts "Note:(ZQ) For more Informations see zynq Electronic Wiki: https://wiki.zynq-electronic.de/display/PD/Project+Delivery"
      puts "------------------------------------------"
    }
  }
  # -----------------------------------------
  ...
  proc hw_blockdesign_create_bd {{args ""}} {
    ...
    if {$run_help} {
      puts "Description:\n\
            ...
            zynq_soc  -> used for 7Series Zynq Design only. Generate 7 Series Zynq with Board Part configuration and Carrier Board extended settings (if available). \n\
            ...
            "
      puts "Syntax:\n\
            ZQ::hw_blockdesign_create_bd \[-bd_name\] \[-msys_local_mem\] \[-msys_ecc\] \[-msys_cache\] \[-msys_debug_module\] \[-msys_axi_periph\] \[-msys_axi_intc\] \[-msys_clk\] \[-help\]\n\
            "
      puts "Returns:\n\
              No return value.\n\
            "
      puts "Usage:\n\
              ...
              -------------------------\n\
              \[-help\]               print help\n\
              \[-bd_name\]            Use one of the predefined names (def:fsys): fsys, msys, zynq_soc, zusys \n\
      ...
      if {[catch {ZQ::VIV::create_new_blockdesign $bd_name $m_settings } result]} { puts "Error:(ZQ) Script (ZQ::VIV::create_new_blockdesign) failed: $result."; return -code error}
    }
  }
  # -----------------------------------------
  ...
  proc hw_blockdesign_export_tcl {{args ""}} {
    ...
    if {$run_help} {
      puts "Description:\n\
            ...
            File destination is $ZQ::BD_PATH or ${ZQ::BD_PATH}/${ZQ::SHORTDIR}/, if sub-folder exists. \n\
            If ${ZQ::BD_PATH}/${ZQ::SHORTDIR}/  exists, Block Designs from $ZQ::BD_PATH are ignored on project creation. \n\
            Attention: Open block-design will be saved automatically before export is run.\
            "
      puts "Syntax:\n\
            ZQ::hw_blockdesign_export_tcl \[-no_mig_contents\] \[-no_validate\] \[-mod_tcl\] \[-svntxt <arg>\] \[-board_part_only\] \[-help\]\n\
            "
      puts "Returns:\n\
              No return value.\n\
            "
      puts "Usage:\n\
              ...
              \[-board_part_only\]  Export for this bord part only (tcl is stored in ${ZQ::BD_PATH}/${ZQ::SHORTDIR}/). \n\
              \[-mod_tcl\]          TCL Content would be modified with content from $ZQ::BD_PATH\\mod_bd.tcl. If mod_bd.tcl don't exist or all commands inside are commented, nothing is changed. Wrong usage will damaged design functionality!  \n\
              \[-svntxt <arg>\]     Send svn commit with Text <arg>if SVN-versioning is used for the files in $ZQ::BD_PATH.\n\
            "
      puts "Categories:\n \
            ZQ::VIV, ZQ::UTILS, ZQ::EXT\n\
            "
    } else {
      if {[catch {ZQ::VIV::export_blockdesign $no_mig $no_validate $boardpart_only $mod_tcl} result]} { puts "Error:(ZQ) Script (ZQ::VIV::export_blockdesign) failed: $result."; return -code error}
      if {$svn_check} {
        if {[catch {ZQ::EXT::svn_checkin ${ZQ::BD_PATH} $svn_msg} result]} { puts "Error:(ZQ) Script (ZQ::EXT::svn_checkin) failed: $result."; return -code error}
      }
    }
  }
  # -----------------------------------------
  ...
  proc hw_build_design {{args ""}} {
    ...
    if {$run_help} {
      puts "Description:\n\
            ...
            "
      puts "Syntax:\n\
            ZQ::hw_build_design \[-disable_bitgen\] \[-disable_hdf\] \[-disable_mcsgen\] \[-disable_reports\] \[-export_prebuilt\] \[-export_prebuilt_only\] \[-help\]\n\
            "
      puts "Returns:\n\
              No return value.\n\
            "
      ...
      if {$run_build} {
        if {[catch {ZQ::VIV::build_design $bitgen $mcsgen $reportgen $hdfgen} result]} { puts "Error:(ZQ) Script (ZQ::VIV::build_design) failed: $result."; return -code error}
      }
      # copy is done if hsi, sdk or jtag programming is started or
      if {$export_prebuild} {
        if {[catch {ZQ::UTILS::copy_hw_files $ZQ::GEN_HW_DELETEOLDFILES} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::copy_hw_files) failed: $result."; return -code error}
        if {[catch {ZQ::UTILS::copy_hw_reports} result]} { puts "Info:(ZQ) No Hardware Reports found."}
      }
    }
  }
  # -----------------------------------------
    ...
    if {$run_help} {
      puts "Description:\n\
            ...
            *.hdef and *.sysdef are ignored if BD-Name is fsys (Without processor system). \n\
            "
      puts "Syntax:\n\
            ZQ::sw_run_hsi \[-run_only\] \[-prebuilt_hdf <arg>\] \[-no_hsi\] \[-no_bif\] \[-no_bin\] \[-no_bitmcs\] \[-clear\] \[-help\]\n\
            "
      puts "Returns:\n\
              No return value.\n\
            "
      ...
      if {$run_clear} {
        if {[catch {ZQ::UTILS::clean_workspace_hsi} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::clean_workspace_hsi) failed: $result."; return -code error}
      }
      if {$run_hsi} {
        if {$run_copy} {
          if {$run_prebuilt} {
            if {[catch {ZQ::UTILS::generate_workspace_hsi $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::generate_workspace_hsi) failed: $result."; return -code error}
            ...
            if {[catch {ZQ::UTILS::copy_hw_files $ZQ::GEN_HW_DELETEOLDFILES} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::copy_hw_files) failed: $result."; return -code error}
            if {[catch {ZQ::UTILS::copy_hw_reports} result]} { puts "Info:(ZQ) No Hardware Reports found. "}
            if {[catch {ZQ::UTILS::generate_workspace_hsi} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::generate_workspace_hsi) failed: $result."; return -code error}
          }
        }
        if {[catch {ZQ::EXT::run_hsi} result]} { puts "Error:(ZQ) Script (ZQ::EXT::run_hsi) failed: $result."; return -code error}
      }
      if {$ZQ::IS_zynq_soc || $ZQ::IS_ZUSYS || $force_bin} {
        #.bif and .bin only on zynq systems
        if {$run_bif} {
          if {[catch {ZQ::EXT::generate_bif_files $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::EXT::generate_bif_files) failed: $result."; return -code error}
        }
        if {$run_bin} {
          if {[catch {ZQ::EXT::generate_bootbin $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::EXT::generate_bootbin) failed: $result."; return -code error}
        }
      } elseif {$ZQ::IS_MSYS} {
        if {$run_bitmcs} {
          if {[catch {ZQ::EXT::generate_app_bit_mcs $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::EXT::generate_app_bit_mcs) failed: $result."; return -code error}
        }
      } else {
          puts "Info:(ZQ) Boot.bif and Boot.bin only for Zynq-FPGAs available. <app>.bit and <app>.mcs only for MicroBlaze available. System will be checked with block design name, current BD file name is $ZQ::BD_TCLNAME .Use:"
          puts "\"zynq_soc_bd.tcl\" for Systems with Zynq "
  ...
  proc sw_run_sdk {{args ""}} {
    ...
    if {$run_help} {
      puts "Description:\n\
            Start SDK project in external folder $ZQ::WORKSPACE_SDK_PATH\n\
            Attention: If you use VIVADO GUI Command (File->Export-> Export Hardware..(Include Bit-file!) or File->Launch SDK) to Update or open SKD set new export path and workspace:  $ZQ::WORKSPACE_SDK_PATH\n\
            Copy HW File and reports from the vivado project to the prebuilt folder if -prebuild_hdf <arg> isn't set (default)\n\
            *.hdef and *.sysdef are ignored if BD-Name is fsys (Without processor system). \n\
            "
      puts "Syntax:\n\
            ZQ::sw_run_sdk \[-open_only\] \[-update_hdf_only\] \[-prebuilt_hdf <arg>\] \[-clear\] \[-help\]\n\
            "
      puts "Returns:\n\
              No return value.\n\
            "
      ...
      if {$run_clear} {
        if {[catch {ZQ::UTILS::clean_workspace_sdk} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::clean_workspace_sdk) failed: $result."; return -code error}
      }
      if {$run_copy} {
        if {$run_prebuilt} {
          if {[catch {ZQ::UTILS::generate_workspace_sdk $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::generate_workspace_sdk) failed: $result."; return -code error}
        } else {
          if {[catch {ZQ::UTILS::copy_hw_files $ZQ::GEN_HW_DELETEOLDFILES} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::copy_hw_files) failed: $result."; return -code error}
          if {[catch {ZQ::UTILS::copy_hw_reports} result]} { puts "Info:(ZQ) No Hardware Reports found."}
          if {[catch {ZQ::UTILS::generate_workspace_sdk} result]} { puts "Error:(ZQ) Script (ZQ::UTILS::generate_workspace_sdk) failed: $result."; return -code error}
        }
      }
      if {$start_sdk} {
        if {[catch {ZQ::EXT::run_sdk} result]} { puts "Error:(ZQ) Script (ZQ::EXT::run_sdk) failed: $result."; return -code error}
      }
    }
  }
  # -----------------------------------------
  ...
  proc pr_program_flash_binfile {{args ""}} {
    ...
    if {$run_help} {
      puts "Description:\n\
            ...
            "
      puts "Syntax:\n\
            ZQ::pr_program_flash_binfile \[-no_reboot\] \[-used_board <arg>\] \[-swapp <arg>\] \[-available_apps\] \[-used_basefolder_binfile\] \[-help\]\n\
      ...
      puts "Usage:\n\
              ...
              \[-used_basefolder_binfile\]  Use base-folder bin-file ($ZQ::BASEFOLDER). Should be only one *.bin!\n\
              \[-help\]                     Print help\n\
            "
      puts "Categories:\n \
            ZQ::EXT, ZQ::VLAB\n\
      ...
      if {$print_available_apps} {
         if {[catch {ZQ::EXT::get_available_apps $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::EXT::get_available_apps) failed: $result."; return -code error}
      } else {
        if {$appname eq ""} { puts "Error:(ZQ) No App name is selected see \[pr_program_flash_binfile -help\]: $result."; return -code error}
        if {$prebuilt_name ne ""} {
          set id "[ZQ::BDEF::find_id $prebuilt_name]"
          set zynqflashtyp_int [ZQ::BDEF::get_zynqflashtyp $id 0]
        } else {
          set zynqflashtyp_int $ZQ::ZYNQFLASHTYP
        }
        ...
        if {$::env(SDK_AVAILABLE) && $check_zynqflash && $use_sdk_flash} {
          if {[catch {set return_filename [ZQ::EXT::excecute_zynq_flash_programming $use_basefolder $appname $prebuilt_name]} result]} { puts "Error:(ZQ) Script (ZQ::EXT::excecute_zynq_flash_programming) failed: $result."; return -code error}
          if {$reboot} {
            set hw_close true
            if {[catch {set hw_close [ZQ::VLAB::hw_open_jtag]} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_open_jtag) failed: $result."; return -code error}
            if {[catch {ZQ::VLAB::hw_fpga_boot_from_memory $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_fpga_boot_from_memory) failed: $result."; return -code error}
            if {$hw_close} {
              if {[catch {ZQ::VLAB::hw_close_jtag} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_close_jtag) failed: $result."; return -code error}
            }
          }
        } else {
          set hw_close true
          if {[catch {set hw_close [ZQ::VLAB::hw_open_jtag]} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_open_jtag) failed: $result."; return -code error}
          if {[catch {ZQ::VLAB::hw_create_flash_device $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_create_flash_device) failed: $result."; return -code error}
          if {[catch {set return_filename [ZQ::VLAB::hw_program_fpga_flash $use_basefolder "" bin $appname $prebuilt_name]} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_program_fpga_flash) failed: $result."; return -code error}
          if {$reboot} {
            if {[catch {ZQ::VLAB::hw_fpga_boot_from_memory $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_fpga_boot_from_memory) failed: $result."; return -code error}
          }
          if {$hw_close} {
      ...
      puts "Info: (ZQ) Programming elapsed time: $timeelapsed seconds"
    }
  ...
  proc pr_program_flash_mcsfile {{args ""}} {
    ...
    if {$run_help} {
      puts "Description:\n\
            ...
            "
      puts "Syntax:\n\
            ZQ::pr_program_flash_mcsfile \[-no_reboot\] \[-used_board <arg>\] \[-unused_io_termination <arg>\] \[-swapp <arg>\] \[-available_apps\] \[-used_basefolder_mcsfile\] \[-help\]\n\
      ...
      puts "Usage:\n\
              ...
              \[-available_apps\]               Return available software app names from selected the prebuilt boot_images folder.\n\
              \[-used_basefolder_mcsfile\]      Use base-folder mcs-file ($ZQ::BASEFOLDER). Should be only one *.mcs!\n\
              \[-help\]                         Print help\n\
            "
      puts "Categories:\n \
            ZQ::EXT, ZQ::VLAB\n\
      ...
      if {$print_available_apps} {
        ...
        if {[catch {set hw_close [ZQ::VLAB::hw_open_jtag]} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_open_jtag) failed: $result."; return -code error}
        if {[catch {ZQ::VLAB::hw_create_flash_device $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_create_flash_device) failed: $result."; return -code error}
        if {[catch {set return_filename [ZQ::VLAB::hw_program_fpga_flash $use_basefolder $term mcs $appname $prebuilt_name]} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_program_fpga_flash) failed: $result."; return -code error}
        if {$reboot} {
          if {[catch {ZQ::VLAB::hw_fpga_boot_from_memory $prebuilt_name} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_fpga_boot_from_memory) failed: $result."; return -code error}
        }
        if {$hw_close} {
  ...
  proc pr_program_jtag_bitfile {{args ""}} {
    ...
    if {$run_help} {
      puts "Description:\n\
            ...
            "
      puts "Syntax:\n\
            ZQ::pr_program_jtag_bitfile \[-used_board <arg>\] \[-used_basefolder_bitfile\] \[-help\]\n\
            "
      puts "Returns:\n\
              Programming file-name.\n\
            "
      puts "Usage:\n\
              ...
              \[-available_apps\]           Return available software app names from selected the prebuilt boot_images folder.\n\
              \[-used_basefolder_bitfile\]  Use base-folder bit-file ($ZQ::BASEFOLDER). Should be only one *.bit!\n\
              \[-help\]                     Print help\n\
            "
      puts "Categories:\n \
            ZQ::VLAB\n\
      ...
      if {$print_available_apps} {
        ...
        if {[catch {ZQ::VLAB::hw_open_jtag} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_open_jtag) failed: $result."; return -code error}
        if {[catch {set return_filename [ZQ::VLAB::hw_program_fpga_device $use_basefolder $appname $prebuilt_name]} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_program_fpga_device) failed: $result."; return -code error}
        if {$hw_wasclosed} {
          if {[catch {ZQ::VLAB::hw_close_jtag} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_close_jtag) failed: $result."; return -code error}
        }        
      }   
  ...
  proc pr_init_hardware_manager {{args ""}} {
    ...
    if {$run_help} {
      puts "Description:\n\
            ...
            "
      puts "Syntax:\n\
            ZQ::pr_init_hardware_manager \[-help\]\n\
            "
      puts "Returns:\n\
              No return value.\n\
            "
      ...
      puts "Categories:\n \
        ...
        if {[catch {ZQ::VLAB::hw_open_jtag} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_open_jtag) failed: $result."; return -code error}
        if {${ZQ::FPGAFLASHTYP} ne "NA"} {
          if {[catch {ZQ::VLAB::hw_create_flash_device} result]} { puts "Error:(ZQ) Script (ZQ::VLAB::hw_create_flash_device) failed: $result."; return -code error}
        }
    }
  }
  # -----------------------------------------
  ...
  proc util_zip_project {{args ""}} {
    ...
    if {!$manual_name} {
      ...
      #projectname 
      set zipname "${zipname}-${ZQ::VPROJ_NAME}"
      if {$remove_prebuilt} {
    ...
    if {$run_help} {
      ...
      if {$save_all} {
        if {[catch {ZQ::EXT::zip_project $zipname} result]} { puts "Error:(ZQ) Script (ZQ::EXT::zip_project) failed: $result."; return -code error}
      } else {
        ...
        if {[llength $ZQ::ZIP_IGNORE_LIST] > 0} {
          set excludelist []
          foreach entry $ZQ::ZIP_IGNORE_LIST {
            if {[lindex $entry 0]==0} {
              #only id0 objects
              ...
              #only id1 objects
              set find []
              catch {set find [glob -join -dir $ZQ::BASEFOLDER [lindex $entry 1]]}
              foreach el $find {
                set sl_start [expr [string length $ZQ::BASEFOLDER]+1]
        ...
        if {[catch {ZQ::EXT::zip_project $zipname $excludelist} result]} { puts "Error:(ZQ) Script (ZQ::EXT::zip_project) failed: $result."; return -code error}
      }
    }
  }
  # -----------------------------------------
  ...
  namespace eval ADV {
    ...
    proc beta_util_sdsoc_project {{args ""}} {
      ...
      if {$run_help} {
        puts "Description:\n\
              ...
              Generate SDSOC project structure in $ZQ::SDSOC_PATH.\n\
              ...
              "
        puts "Syntax:\n\
              ZQ::ADV::beta_util_sdsoc_project \[-check_only\] \[-start_sdsoc\] \[-help\]\n\
              "
        puts "Returns:\n\
                No return value.\n\
              "
        puts "Usage:\n\
                ...
                \[-start_sdsoc\]       Start SDSOC with workspace: $ZQ::SDSOC_PATH \n\
                \[-help\]              print help\n\
              "
        puts "Categories:\n \
              ZQ::SDSOC, ZQ::EXT\n\
        ...
        if {$check_sdsoc} {
          if {[catch {ZQ::SDSOC::check_and_modify_vivado_project true} result]} { puts "Error:(ZQ) Script (ZQ::SDSOC::check_and_modify_vivado_project) failed: $result."; return -code error}
        } elseif {$start_sdsoc} {
          if {[catch {ZQ::EXT::run_sdsoc} result]} { puts "Error:(ZQ) Script (ZQ::EXT::run_sdsoc) failed: $result."; return -code error}
        } else {
          if {[catch {ZQ::SDSOC::create_sdsoc_structure} result]} { puts "Error:(ZQ) Script (ZQ::SDSOC::create_sdsoc_structure) failed: $result."; return -code error}
          if {[catch {ZQ::SDSOC::check_and_modify_vivado_project false} result]} { puts "Error:(ZQ) Script (ZQ::SDSOC::check_and_modify_vivado_project) failed: $result."; return -code error}
           #todo rebuild project files
          if {[catch {ZQ::SDSOC::export_vivado_sdsoc_project} result]} { puts "Error:(ZQ) Script (ZQ::SDSOC::export_vivado_sdsoc_project) failed: $result."; return -code error}
          if {[catch {ZQ::SDSOC::create_sdsoc_pfm} result]} { puts "Error:(ZQ) Script (ZQ::SDSOC::create_sdsoc_pfm) failed: $result."; return -code error}
          puts "Info:(ZQ) Create SDSOC project finished"
        }
      }
    }
    # -----------------------------------------
      ...
      if {$run_help} {
        puts "Description:\n\
              ...
              To restore project after permanent modification do: \n\
                Delete ${ZQ::BD_PATH}/*.tcl. \n\
                Rename ${ZQ::BD_PATH}/*.tcl_backup into ${ZQ::BD_PATH}/*.tcl. \n\
                Delete ${ZQ::BOARDDEF_PATH}/*_board_files_mod.csv. \n\
              "
        puts "Syntax:\n\
              ZQ::ADV::beta_hw_remove_board_part \[-permanent\] \[-help\]\n\
              "
        puts "Returns:\n\
                No return value.\n\
              "
        ...
        if {[catch {ZQ::VIV::design_exclude_boarddef $temp_only} result]} { puts "Error:(ZQ) Script (ZQ::VIV::design_exclude_boarddef) failed: $result."; return -code error}
      }
    }
    # -----------------------------------------
    ...
    proc beta_hw_export_rtl_ip {{args ""}} {
      ...
      if {$run_help} {
        puts "Description:\n\
              Export RTL-IP Cores (*.xci), which are not included in a Block-Design to ${ZQ::HDL_PATH}/xci/${ZQ::SHORTDIR}. \n\
              HDL and *.xci files, which include in the folder $ZQ::HDL_PATH are load automatically on project creation. \n\
              "
        puts "Syntax:\n\
              ZQ::ADV::beta_hw_export_rtl_ip \[-help\]\n\
              "
        puts "Returns:\n\
                No return value.\n\
              "
        ...
        if {[catch {ZQ::VIV::export_xci} result]} { puts "Error:(ZQ) Script (ZQ::VIV::export_xci) failed: $result."; return -code error}
      }
    }
  }
  # -----------------------------------------
  ...
  # -----------------------------------------
  # -----------------------------------------
  puts "Info:(ZQ) Load User Command scripts finished"
}
