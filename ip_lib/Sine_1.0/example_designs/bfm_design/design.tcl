proc create_ipi_design { offsetfile design_name } {
	...
	set Sine_0 [ create_bd_cell -type ip -vlnv xilinx.com:user:Sine:1.0 Sine_0]
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI_LITE] [get_bd_intf_pins Sine_0/ctrl_saxi]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/M_AXI_LITE_ACLK] [get_bd_pins Sine_0/ctrl_saxi_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/M_AXI_LITE_ARESETN] [get_bd_pins Sine_0/ctrl_saxi_ARESETN]
	...
	set offset_file "${bd_path}/Sine_v1_0_tb_include.vh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef Sine_v1_0_tb_include_vh_"
	puts $fp "`define Sine_v1_0_tb_include_vh_\n"
	...
	puts $fp "`define ctrl_saxi_SLAVE_ADDRESS ${offset_hex}"

	puts $fp "`endif"
	close $fp
}

set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores xilinx.com:user:Sine:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/Sine_v1_0_tb.v
...
for { set i 1 } { 1 } { incr i } {
	set design_name "Sine_v1_0_bfm_${i}"
...
remove_files -quiet -fileset sim_1 Sine_v1_0_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top Sine_v1_0_tb [get_filesets sim_1]
